...
I was at Rust Week where Niko Matsakis gave his talk "Our Vision for Rust"
in which he made a profound and insightful statement:

**Rust is a language for building *foundational software*.**

That deeply struck me.

I highly recommend you read it his blog post titled ["Rust in 2025: Targeting foundational software"](https://smallcultfollowing.com/babysteps/blog/2025/03/10/rust-2025-intro/), which is a great
summary on the topic.
I wanted to expand on the idea and share what this means to corrode (and perhaps to a wider extend to Rust in the industry).

## The Issue With "Systems Programming"

First off, do we really need another different term?
Why can't we just stick to "systems programming"?
Doesn't that imply <<that we build "systems" (for whatever that means)?>>
After all, many people still think of Rust as a systems programming language first and foremost.

I believe the framing is all wrong.
Systems programming <<is a term with a huge historic baggage>> that feels limiting/prohibitive.
<<It makes a distinction between systems programming and "other kinds of programming.">>

The mindset "We are not a systems programming company so we don't need Rust" is common, but limiting

If I may be candid for a moment, I believe well-known systems-programming domains 
have a tendency to be toxic. Even the best developers in the world made that experience
Here is Glauber Costa, founder of Torso, who worked at Red Hat, Parallels, ScyllaDB, and Datadog
on schedulers, databases and performance optimizations. He knows what he's doing.
https://www.youtube.com/watch?v=biBLEKm2dtY&t=307s
...
Just imagine how many capable developers got discouraged by similar feedback or never
even tried to contribute to the Linux kernel in the first place because of such comments.

I find that ironic because <<people said the same about Linux being a toy project in the beginning>>.
...
To me, it's about breaking down the barriers to entry and making larger parts of the software stack accessible to everyone. 
Niko describes it as ["trying to defy the 'common wisdom' about what the tradeoffs have to be."](https://smallcultfollowing.com/babysteps/blog/2025/05/15/10-years-of-rust/)

That is also where the term "corrode" stems from the Latin word "corrodere", which means "to gnaw to bits, wear away," [Source](https://www.etymonline.com/search?q=corrode).
...
Graydon Hoare: "Rust is a tool for building other infrastructure"
...
"the world needs robust and reliable infrastructure, and the infrastructure we had was not up to the task."

## Where Rust Shines

In conversations with potential customers, one key aspect that comes up with Rust a lot is 
<<this sentiment that Rust is a systems programming language>>.
...
People keep asking me how Rust could help them. After all, Rust is just a "systems programming language."
...
This is, in my opinion, the core value proposition of Rust: to build things <<which need to last and should never fail>>

Rust is day-2-language: All of the problems that you have during the lifecycle of your application surface early in development
...
This mindset takes away the focus from Rust as a C++ replacement and also explains why so many
teams which use languages like Python, TypeScript, and Kotlin are attracted by Rust.

## Why Focus On Foundational Software?

But don't we just replace "Systems Programming" with "Foundational Software"?
<<Isn't it limiting to use the term "Foundational Software" as well?>>

Crucially foundational software is different from low-level software and systems software
For my client it's all foundational.
For example, building a data plane is foundational.
Processing media is foundational.

Rust serves as a catalyst: companies start using it for critical software but then,
 they get more comfortable with the language, expand into using it in other areas of their business:
...
> Marc Brooker, engineer at Amazon Web Services in Seattle
> https://lobste.rs/s/a46njy/rust_2025_targeting_foundational#c_zictwg

That fully aligns with my experience: I find that teams become ambitious after a while.
Reaching for loftier goals because they can.
The fact they don't have to deal with security issues anymore enables them better affordances.
From my conversation with other Rustaceans, we all made the same observation: suddenly we can build more ambitious projects that we never dared tackling before, such as writing a CPU emulator because we can. For fun.
...
It's software that a organizations deem critical for their success. 
It might be:
...
- a storage layer
- a runtime for large data imports 

and many, many more.

All of these things power organizations and **must not fail** or at least do so **gracefully**.
My clients and the companies listed on the [podcast page] all have one thing in common: 
...
Rust is a language that enables people to move across domain boundaries: from embedded to cloud, from data science to developer tooling. 
I don't know any other language that's so versatile. 
If you know Rust you can program simple things in all of these domain

- It is useful in situations, where the ["worse is better" philosophy falls apart](https://dreamsongs.com/Files/worse-is-worse.pdf): 

> With the right thing, designers are equally concerned with simplicity, correctness, consistency, and completeness.

Rust is about building the "right thing."
...
I believe that we should shift the focus away from memory safety (which these languages also have) and instead focus on the explictness and expressiveness and the ecosystem of Rust that is highly competitive with these languages.
...
Dropbox was one of the first production users of Rust. They built their storage layer on top of it.
...
To build a truly industrial-strength ecosystem, we need to remember the professional software lifecycle,
...
To build foundational software, you need guarantees that vulnerabilities get fixed and that the ecosystem evolves and adapts the customer's needs.
The ecosystem is still mostly driven by volunteers who work on important parts of the ecosystem in their freetime.
There is a thing to be said about supply-chain security and sustainability in the ecosystem.

## A Language For Professionals

Rust inherits pragmatism from C++ and purism from Haskell.
Pragmatic purism is not an oxymoron.
...
We are building systems that will be used by people who may not even know they are using them, but who will depend on them every day. Critical infrastructure.
To me it also means to build sustainable software that stays within its means and is concerned about low resource usage.
Systems where precision and correctness matter.
Across languages boundaries and up and down the stack.
With all the escape hatches of writing simple application code on top (link: prototyping)
A while ago I read a post titled "Java for everything." It made me wonder if Rust could also be used for everything
Rust is a language for decades.

My mission is to be a part of this shift.
