...
They see the benefit of reliable software, but they don't see how Rust could help them. 
...
In practice, my clients use Rust for a **much broader range of software**, not just kernel drivers and embedded code!
...
What all clients have in common, however, is that the services they build with Rust are **foundational to their core business**.
...
To build a truly industrial-strength ecosystem, we need to remember the professional software lifecycle, which is often measured in decades.
Stability plays a big role in that.
The fact that Rust has stable editions and a language specification is crucial for industry adoption. 

But Rust is more than just a compiler and its standard library: the tooling and wider ecosystem are equally important.
...
There is more to be said about supply-chain security and sustainability in a future post. 
...
Rust inherited the pragmatism from C++ and the purism from Haskell.
